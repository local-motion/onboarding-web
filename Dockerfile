...
CMD ["/bin/sh", "-c", "echo $HTPASSWD >/etc/nginx/conf.d/.htpasswd && envsubst '$AUTHSWITCH'< /etc/nginx/conf.d/default_conf.template > /etc/nginx/conf.d/default.conf && exec nginx -g 'daemon off;'"]
